<?php
	...
	trait Flash {
		...
		protected function flash__init($uniq_id) {
		...
		protected function flash__set($key, $value=null) {
			if((null !== $value) || isset(self::$flash__storage[ $this->flash__uniq_id ][ $key ])) {
				self::$flash__storage[ $this->flash__uniq_id ][ $key ] = $value;
			}
		}
		
		protected function flash__invalidate_cache() {
			api::cache()->delete( self::flash__hash_uniq_id( $this->flash__uniq_id ) );
		}
		...
		public function toArray() {
			...
			if(isset(self::$flash__storage[ $this->flash__uniq_id ])) {
				return (array)self::$flash__storage[ $this->flash__uniq_id ];
			}
